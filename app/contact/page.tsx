...
const Contact = () => {
  ...
  return (
    <motion.div initial='hidden' animate='show' variants={navLayout} className='relative h-screen lg:h-[92.5vh] lg:overflow-hidden  '>
      ...
      <div className='absolute  inset-x-0 inset-y-0   '>
        ...
        <div className='flex flex-col items-center z-10 bg-slate-900/40 h-full w-full rounded-md font-mono px-2  backdrop-blur-md pt-4'>
          ...
          <div className='  grid  w-full sm:w-[80%] h-[80%] rounded-xl '>
            ...
            <span className=' pt-1 text-xl text-center lg:text-2xl font-mono'>Make sure to check out my socials, or send me an email!😁  </span>
                 <span className='   text-slate-500 text-xs pt-1'>Or don&apos;t, I mean, whatever is fine :(</span>
            </div>
            <div className='  border border-lime-500/50 hover:border-none backdrop-blur-md bg-slate-100/5 justify-center flex px-6 sm:px-8 hover:scale-110 transition-all duration-300 hover:z-10 '>
                ...
                <Image src='/github-mark-white.svg' width={50} height={50} className='ml-2 lg:ml-3 '  alt='github icon'/>
                <p className='text-lg lg:text-2xl font-bold text-lime-500 font-mono'>Github</p>
                </a>
                ...
                <Image src='/linkedin.svg'  width={50} height={50} className='ml-4 lg:ml-5 '  alt='linkedin icon'/>
                <p className='text-lg lg:text-2xl font-bold text-lime-500 font-mono'>LinkedIn</p>
                </a>
                ...
                <Image src='/email.svg'  width={50} height={50} className=' lg:ml-2 '  alt='Mail icon'/>
                <p className='text-lg lg:text-2xl font-bold text-lime-500 font-mono'>Email</p>
                </a>
                    </div>
            </div>
            ...
            <div className='w-0 h-2 sm:w-full lg:h-10'></div>
          <p className='text-lime-500 font-bold text-xl lg:text-2xl font-mono text-center '>Animation Gap: {changeAnimationGap.toFixed(2)}rem</p>
            <div className='flex flex-row justify-center font-orbitron '>
          <button className=' z-100 border border-lime-500/50 text-lime-500/90 rounded-md p-0 sm:p-1 px-4 sm:px-4 active:bg-green-500 active:scale-90 hover:scale-110 transition-all bg-cyan-900 shadow-inner shadow-slate-900' onClick={() => setChangeAnimationGap(changeAnimationGap -0.2)}>{'<<<'}</button>
          <button className={` z-100 border border-lime-500/50 text-lime-500/90 rounded-md p-0 sm:p-1 px-4 sm:px-4 mx-2 hover:scale-110 transition-all active:bg-green-500 active:scale-95 shadow-inner shadow-slate-900 ${animationStop ?'bg-cyan-900 ' :'bg-red-800/70 text-slate-100 font-bold'}`} onClick={()=> setAnimationStop(!animationStop)}>{!animationStop ? 'Stop Animation' : 'Start Animation'}</button>
          <button className=' z-100 border border-lime-500/50 text-lime-500/90 rounded-md p-0 sm:p-1 px-4 sm:px-4 active:bg-green-500 active:scale-90 hover:scale-110 transition-all bg-cyan-900 shadow-inner shadow-slate-900 ' onClick={() => setChangeAnimationGap(changeAnimationGap +0.2)} >{'>>>'}</button>
            </div>
          </div>
